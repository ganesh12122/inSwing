...
class MatchEvent(Base):
    ...
    # Event information
    event_type = Column(
        Enum('boundary', 'wicket', 'milestone', 'highlight', 'comment', 'innings_change', 'toss', name='event_type'),
        nullable=False,
        index=True
    )
    
    # Event metadata (flexible JSON structure)
    meta = Column(JSON, nullable=False)  # Contains player_id, runs, description, timestamp, etc.
    ...
    def __repr__(self):
        return f"<MatchEvent(id={self.id}, match={self.match_id}, type={self.event_type})>"
