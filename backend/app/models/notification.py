...
class Notification(Base):
    ...
    notification_type = Column(
        Enum('match_start', 'wicket', 'milestone', 'mentioned', 'comment', 'system', name='notification_type'),
        nullable=False,
        index=True
    )
    
    # Notification payload (flexible JSON structure)
    payload = Column(JSON, nullable=False)  # Contains match_id, player_id, message, action_url, etc.
