from sqlalchemy import Column, String, DateTime, Boolean, Enum, Integer, Float, ForeignKey, Text, JSON
...
class Match(Base):
    ...
    host_user_id = Column(MySQLCHAR(36), ForeignKey("users.id", ondelete="CASCADE"), nullable=False, index=True)
    
    # Match information
    match_type = Column(Enum('quick', 'friendly', 'tournament', name='match_type'), default='quick', nullable=False)
    ...
    # Match status
    status = Column(
        Enum('created', 'toss_done', 'live', 'finished', 'cancelled', name='match_status'),
        default='created',
        nullable=False,
        index=True
    )
    ...
    # Relationships
    host = relationship("User", back_populates="hosted_matches", foreign_keys=[host_user_id])
    players = relationship("PlayersInMatch", back_populates="match", cascade="all, delete-orphan")
