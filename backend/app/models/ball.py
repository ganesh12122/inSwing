from sqlalchemy import Column, String, DateTime, Integer, Float, ForeignKey, Enum, JSON
...
class Ball(Base):
    ...
    innings_id = Column(MySQLCHAR(36), ForeignKey("innings.id", ondelete="CASCADE"), nullable=False, index=True)
    ...
    # Players involved
    batsman_id = Column(MySQLCHAR(36), ForeignKey("users.id", ondelete="SET NULL"), nullable=True, index=True)
    non_striker_id = Column(MySQLCHAR(36), ForeignKey("users.id", ondelete="SET NULL"), nullable=True, index=True)
    bowler_id = Column(MySQLCHAR(36), ForeignKey("users.id", ondelete="SET NULL"), nullable=True, index=True)
    
    # Scoring information
    runs_off_bat = Column(Integer, default=0, nullable=False)
    extras_type = Column(
        Enum('wide', 'no_ball', 'bye', 'legbye', name='extras_type'),
        nullable=True
    )
    ...
    # Wicket information
    wicket_type = Column(
        Enum('bowled', 'caught', 'runout', 'lbw', 'stumped', 'hit_wicket', name='wicket_type'),
        nullable=True
    )
    dismissal_info = Column(JSON, nullable=True)  # Detailed dismissal information
    
    # Additional metadata
    client_event_id = Column(String(100), nullable=True, index=True)  # For offline sync idempotency
