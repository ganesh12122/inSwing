...
async def create_user(
    ...
    if current_user and current_user.role != "admin":
        logger.warning(
            "Non-admin user attempted to create user", user_id=current_user.id
        )
        raise HTTPException(
...
async def get_user_profile(
    ...
    if not user:
        logger.warning(
            "User not found", requested_user_id=user_id, current_user_id=current_user.id
        )
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="User not found"
        )
...
async def update_user_profile(
    ...
    if user_update.email is not None:
        ...
        if existing_user:
            logger.warning(
                "Email already taken", email=user_update.email, user_id=user_id
            )
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
...
async def search_users(
    ...
    results = []
    for user in users:
        results.append(UserProfileResponse(user=user, profile=profile_map.get(user.id)))
