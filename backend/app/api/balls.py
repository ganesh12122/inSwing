...
async def create_innings(
    ...
    if not match:
        logger.warning("Match not found for innings creation", match_id=match_id, user_id=current_user.id)
        raise HTTPException(
    ...
    if match.host_user_id != current_user.id:
        logger.warning("Unauthorized innings creation", 
                      match_id=match_id, 
    ...
    logger.info("Innings created successfully", 
...
async def get_innings(
    ...
    if not match:
        logger.warning("Match not found for innings retrieval", match_id=match_id, user_id=current_user.id)
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="Match not found"
        )
    
    innings = db.query(Innings).filter(Innings.match_id == match_id).order_by(Innings.created_at).all()
    
    logger.info("Innings retrieved", 
                match_id=match_id, 
...
async def record_ball(
    ...
    match = db.query(Match).filter(Match.id == match_id).first()
    if not match:
        logger.warning("Match not found for ball recording", match_id=match_id, user_id=current_user.id)
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="Match not found"
        )
    ...
    if not innings:
        logger.warning("Innings not found for ball recording", 
    ...
    # Check if user is the host
    if match.host_user_id != current_user.id:
        logger.warning("Unauthorized ball recording", 
                      match_id=match_id, 
                      user_id=current_user.id,
                      host_id=match.host_user_id)
        raise HTTPException(
    ...
    if ball_data.client_event_id:
        existing_ball = db.query(Ball).filter(
            Ball.innings_id == innings_id,
            Ball.client_event_id == ball_data.client_event_id
        ).first()
        
        if existing_ball:
            logger.info("Ball already recorded (idempotent)", 
    ...
    if ball_data.batsman_id:
        batsman_in_match = db.query(PlayersInMatch).filter(
    ...
    if ball_data.bowler_id:
        bowler_in_match = db.query(PlayersInMatch).filter(
            PlayersInMatch.match_id == match_id,
    ...
    if (innings.wickets >= 10 or 
        innings.overs_bowled >= innings.overs_allocated):
    ...
    logger.info("Ball recorded successfully", 
                ball_id=ball.id,
                innings_id=innings_id,
                match_id=match_id,
...
async def get_balls(
    ...
    match = db.query(Match).filter(Match.id == match_id).first()
    if not match:
        logger.warning("Match not found for balls retrieval", match_id=match_id, user_id=current_user.id)
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="Match not found"
        )
    ...
    if not innings:
        logger.warning("Innings not found for balls retrieval", 
                      innings_id=innings_id, 
                      match_id=match_id,
                      user_id=current_user.id)
        raise HTTPException(
...
@router.put("/{match_id}/innings/{innings_id}/balls/{ball_id}", response_model=BallResponse)
async def update_ball(
    ...
    match = db.query(Match).filter(Match.id == match_id).first()
    if not match:
        logger.warning("Match not found for ball update", match_id=match_id, user_id=current_user.id)
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="Match not found"
        )
    ...
    if not innings:
        logger.warning("Innings not found for ball update", 
                      innings_id=innings_id, 
                      match_id=match_id,
                      user_id=current_user.id)
        raise HTTPException(
    ...
    # Find the ball
    ball = db.query(Ball).filter(
        Ball.id == ball_id,
        Ball.innings_id == innings_id
    ).first()
    
    if not ball:
        logger.warning("Ball not found for update", ball_id=ball_id, user_id=current_user.id)
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
    ...
    # Check if user is the host
    if match.host_user_id != current_user.id:
        logger.warning("Unauthorized ball update", 
                      match_id=match_id, 
                      user_id=current_user.id,
                      host_id=match.host_user_id)
        raise HTTPException(
    ...
    logger.info("Ball updated successfully", 
                ball_id=ball_id,
                innings_id=innings_id,
                match_id=match_id,
                runs_diff=runs_diff,
