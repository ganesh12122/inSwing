...
async def get_bowling_leaderboard(
    ...
    bowling_statement = (
        select(
            ...
            func.count(Ball.id)
            .filter(Ball.extras_type.is_(None))
            .label("balls_bowled"),
...
async def get_matches_hosted_leaderboard(
    ...
    for i, stat in enumerate(hosted_stats, 1):
        avg_duration_minutes = (
            (float(stat.avg_match_duration_seconds) / 60)
            if stat.avg_match_duration_seconds
            else 0
        )
    ...
    logger.info(
        "Matches hosted leaderboard retrieved",
        period=period,
        limit=limit,
        entries_count=len(entries),
        user_id=current_user.id,
...
async def get_player_rating_leaderboard(
    ...
    logger.info(
        "Player rating leaderboard retrieved",
        period=period,
        limit=limit,
        entries_count=len(entries),
        user_id=current_user.id,
