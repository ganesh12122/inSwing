...
class ConnectionManager:
    ...
    async def broadcast_to_match(
        self, message: dict, match_id: str, exclude_user_id: str = None
    ):
        ...
        for user_id in self.match_connections[match_id]:
            ...
            if user_id in self.active_connections:
                ...
                try:
                    ...
                    logger.error(
                        "Failed to send match broadcast",
                        user_id=user_id,
                        match_id=match_id,
                        error=str(e),
                    )
    ...
    async def broadcast_to_all(self, message: dict, exclude_user_id: str = None):
        ...
        for user_id, connection in self.active_connections.items():
            ...
            try:
                ...
                logger.error(
                    "Failed to send global broadcast", user_id=user_id, error=str(e)
                )
                disconnected_users.append(user_id)
    ...
    try:
        ...
        while True:
            ...
            try:
                ...
                await manager.send_personal_message(
                    {"type": "error", "message": "Invalid message format"}, user_id
                )
...
async def handle_match_subscription(user_id: str, match_id: str, websocket: WebSocket):
    ...
    logger.info("User subscribed to match updates", user_id=user_id, match_id=match_id)
...
async def handle_match_unsubscription(user_id: str, match_id: str):
    ...
    logger.info(
        "User unsubscribed from match updates", user_id=user_id, match_id=match_id
    )
...
async def handle_ball_update(message: dict, user_id: str):
    ...
    await manager.broadcast_to_match(
        broadcast_message, match_id, exclude_user_id=user_id
    )

    logger.info(
        "Ball update broadcasted",
        user_id=user_id,
        match_id=match_id,
...
async def handle_match_status_update(message: dict, user_id: str):
    ...
    logger.info(
        "Match status update broadcasted",
        user_id=user_id,
        match_id=match_id,
        status=status,
    )
...
async def send_ball_notification(
    match_id: str, ball_data: dict, exclude_user_id: str = None
):
...
async def send_match_status_notification(
    match_id: str, status: str, exclude_user_id: str = None
):
...
@router.websocket("/public/{match_id}")
async def public_websocket_endpoint(websocket: WebSocket, match_id: str):
    """Public WebSocket — no auth required. Read-only match subscription.

    Viewers connect with just a match_id and receive ball_update / match_status_update
    messages in real-time. They cannot send scoring commands.
    """
    viewer_id = f"viewer_{id(websocket)}"

    try:
        await manager.connect(websocket, viewer_id, match_id)
        ...
        # Keep connection alive — only allow ping/pong from viewer
        while True:
            data = await websocket.receive_text()
            try:
                message = json.loads(data)
                ...
                if msg_type == "ping":
                    ...
                    await websocket.send_text(
                        json.dumps(
                            {
                                "type": "error",
                                "message": "Public viewers can only send ping messages",
                            }
                        )
                    )
            except json.JSONDecodeError:
                await manager.send_personal_message(
                    {"type": "error", "message": "Invalid message format"}, viewer_id
                )

    except WebSocketDisconnect:
        manager.disconnect(viewer_id, match_id)
    except Exception as e:
        logger.error("Public WebSocket error", viewer_id=viewer_id, error=str(e))
        manager.disconnect(viewer_id, match_id)


async def send_innings_completion_notification(
    match_id: str, innings_data: dict, exclude_user_id: str = None
):
...
async def send_match_completion_notification(
    match_id: str, match_result: dict, exclude_user_id: str = None
):
