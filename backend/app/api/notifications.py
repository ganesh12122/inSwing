...
async def create_notification(
    ...
    if notification_data.user_id != current_user.id and current_user.role != "admin":
        logger.warning("Unauthorized notification creation attempt", 
                      user_id=current_user.id,
...
async def mark_notification_read(
    ...
    if not notification:
        logger.warning("Notification not found for read marking", 
...
async def delete_notification(
    ...
    if not notification:
        logger.warning("Notification not found for deletion", 
                      notification_id=notification_id,
                      user_id=current_user.id)
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
...
async def cleanup_old_notifications(
    ...
    deleted_count = db.query(Notification).filter(
        ...
        Notification.status == NotificationStatus.READ  # Only delete read notifications
    ).delete()
...
async def send_match_invitation_notification(
    ...
    if not match:
        logger.warning("Match not found for invitation", 
                      match_id=match_id,
                      user_id=current_user.id)
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
    ...
    if match.host_user_id != current_user.id:
        logger.warning("Unauthorized match invitation", 
                      match_id=match_id,
                      user_id=current_user.id,
    ...
    for user_id in user_ids:
        ...
        if not invited_user:
            logger.warning("User not found for invitation", 
                          user_id=user_id,
        ...
        notification = Notification(
            ...
            message=f"{current_user.name} invited you to join a cricket match",
    ...
    logger.info("Match invitation notifications sent", 
                match_id=match_id,
                host_id=current_user.id,
...
async def send_match_update_notification(
    ...
    match = db.query(Match).filter(Match.id == match_id).first()
    if not match:
        logger.warning("Match not found for update notification", 
                      match_id=match_id,
                      user_id=current_user.id)
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
    ...
    if match.host_user_id != current_user.id:
        logger.warning("Unauthorized match update notification", 
                      match_id=match_id,
                      user_id=current_user.id,
                      host_id=match.host_user_id)
        raise HTTPException(
    ...
    players = db.query(PlayersInMatch).filter(
    ...
    logger.info("Match update notifications sent", 
                match_id=match_id,
                host_id=current_user.id,
